...
def efetuar_transferencia() -> None:
    if len(contas) > 0:
        ...
        conta_o: Conta = buscar_conta_numero(numero_o)

        if conta_o:
            numero_d: int = int(input('Informe o número da conta de destino: '))
            conta_d: Conta = buscar_conta_numero(numero_d)

            if conta_d:
                valor: float = float(input('Informe o valor da transferência: '))
                conta_o.transferir(conta_d, valor)
            else:
                print(f'A conta de destino {numero_d} não foi encontranda.')
        else:
            print(f'A conta de origem {numero_o} não foi encontranda.')
    else:
        print('Ainda não existem contas cadastradas.')
    sleep(2)
    menu()
